...
class queueLogic {
    ...
    public function editGroupbuySaleCount($groupbuy_info) {
        ...
        if (!$update) {
            return callback(statecodeClass::ERROR,'更新抢购信息失败groupbuy_id:'.$groupbuy_info['groupbuy_id']);
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function editVoucherState($voucher_list) {
        ...
        foreach ($voucher_list as $store_id => $voucher_info) {
            ...
            if ($update) {
                ...
                return callback(statecodeClass::ERROR,'更新代金券状态失败vcode:'.$voucher_info['voucher_code']);
            }
        }
        return callback(statecodeClass::SUCCESS);
    }
    ...
    public function createOrderUpdateStorage($goods_buy_quantity) {
        ...
        if (!$result) {
            return callback(statecodeClass::ERROR,'变更商品库存与销量失败');
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function updateGoodsFCode($fc_id) {
        ...
        if (!$update) {
            return callback(statecodeClass::ERROR,'更新F码使用状态失败fc_id:'.$fc_id);
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function delCart($cart) {
        if (!is_array($cart['cart_ids']) || empty($cart['buyer_id'])) return callback(statecodeClass::SUCCESS);
        $del = Model('cart')->delCart('db',array('buyer_id'=>$cart['buyer_id'],'cart_id'=>array('in',$cart['cart_ids'])));
        if (!$del) {
            return callback(statecodeClass::ERROR,'删除购物车数据失败');
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function updateGoodsPromotionPriceByGoodsId($goods_id) {
        ...
        if (!$update) {
            return callback(statecodeClass::ERROR,'根据商品ID更新促销价格失败');
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function updateGoodsPromotionPriceByGoodsCommonId($goods_commonid) {
        ...
        if (!$update) {
            return callback(statecodeClass::ERROR,'根据商品公共id更新促销价格失败');
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function createGoodsFCode($param) {
        ...
        if (!empty($insert)) {
            ...
            if (!$insert) {
                return callback(statecodeClass::ERROR,'生成商品F码失败goods_commonid:'.$param['goods_commonid']);
            }
        }
        return callback(statecodeClass::SUCCESS);
    }
    ...
    public function delOrderCountCache($order_info){
        if (empty($order_info)) return callback(statecodeClass::SUCCESS);
    ...
    public function sendVrCode($param) {
        if (empty($param) && !is_array($param)) return callback(statecodeClass::SUCCESS);
        $condition = array();
        ...
        if (empty($code_list)) return callback(statecodeClass::SUCCESS);
        ...
        if (!$result) {
            return callback(statecodeClass::ERROR,'兑换码发送失败order_id:'.$param['order_id']);
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function saveDeliveryOrder($param) {
        if (!is_array($param['order_sn_list'])) return callback(statecodeClass::SUCCESS);
        $data = array();
        ...
        foreach ($param['order_sn_list'] as $order_id => $v) {
            ...
            if (!$insert) {
                return callback(statecodeClass::ERROR,'保存自提站订单信息失败order_sn:'.$v['order_sn']);
            }
        }
        return callback(statecodeClass::SUCCESS);
    }
    ...
    public function sendPickupcode($param) {
        ...
        if (!$result) {
            return callback(statecodeClass::ERROR,'发送提货码短信消息失败order_id:'.$param['order_id']);
        } else {
            return callback(statecodeClass::SUCCESS);
        }
    }
    ...
    public function build_pwdvoucher($t_id){
        $t_id = intval($t_id);
        if($t_id <= 0){
            return callback(statecodeClass::ERROR,'参数错误');
        }
        $model_voucher = Model('voucher');
        //查询代金券详情
        $where = array();
        ...
        if($t_total <= 0){
            return callback(statecodeClass::ERROR,'代金券模板信息错误');
        }
        while($t_total > 0){
            $is_succ = false;
            $insert_arr = array();
    ...
    public function build_pwdRedpacket($t_id){
        ...
        if($t_total <= 0){
            return callback(statecodeClass::ERROR,'红包模板信息错误');
        }
        while($t_total > 0){
            $is_succ = false;
            $insert_arr = array();
