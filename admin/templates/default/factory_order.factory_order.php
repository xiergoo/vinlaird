...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3><?php echo $lang['nc_factory_order'];?></h3>
  ...
  <form method='post' id="form_factory">
    ...
    <table class="table tb-type2">
      <thead>
        <tr class="thead">
          <th class="w36">订单编号</th>
          <th class="w72">工厂名称</th>
          <th class="w72">商品名称</th>
          <th class="w60 align-center">商品数量</th>
          <th class="w60 align-center">应付金额</th>
          <th class="w60 align-center">订单状态</th>
          <th class="w60 align-center">支付状态</th>
          <th class="w60 align-center"><?php echo $lang['nc_handle'];?> </th>
        </tr>
      </thead>
      <tbody>
        <?php if (!empty($output['fo_list']) && is_array($output['fo_list'])) { ?>
        <?php foreach ($output['fo_list'] as $k => $v) {?>
        <tr class="hover edit">
          <td class="align-center"><?php echo $v['fo_id'];?></td>
          <td><?php echo $output['factory_list'][$v['factory_id']]['factory_name'];?></td>
          <td><?php echo $output['goods_list'][$v['goods_id']]['goods_name'];?></td>
          <td class="align-center"><?php echo $v['goods_num']?></td>
          <td class="align-center"><?php echo $v['pay_amount'] ?></td>
          <td class="align-center"><?php echo $v['fo_status'] ?></td>
          <td class="align-center"><?php echo $v['pay_amount'] ?></td>
          <td class="align-center"><a href="<?php echo urlAdmin('factory_order', 'edit', array('fo_id' => $v['fo_id']));?>"><?php echo $lang['nc_edit'];?></a>&nbsp;|&nbsp;<a href="<?php echo urlAdmin('factory_order', 'view', array('fo_id' => $v['fo_id']));?>">查看</a></td>
        </tr>
...
$(function(){
    ...
    $('i[nctype="ajaxfactoryList"]').toggle(
        function(){
            ...
            if (_div.html() == '') {
                $.getJSON('index.php?act=factory&op=get_fo_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {
